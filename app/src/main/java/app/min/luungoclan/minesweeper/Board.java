...
public class Board {
    ...
    public void reset(){
        ...
        this.shuffleBomb(this.bombCount);
    ...
    private void setPositions() {
        int horizontalOffset = cellSpriteSheet.getWidth()/3;
        for(int i = 0; i<this.grid.length; i++){
            for(int j = 0; j<this.grid.length ; j++){
                this.grid[i][j].setX(horizontalOffset*i);
                this.grid[i][j].setY(horizontalOffset*j+200);
            }
        }
    }
                ...
                for(int i = this.grid[x][y].getX()-1; i<=this.grid[x][y].getX()+1; i++){
                    for(int j = this.grid[x][y].getY()-1; j<=this.grid[x][y].getY()+1; j++){
                        if(i>=0 && i<this.grid.length && j>= 0 && j<this.grid.length ){
                            this.grid[x][y].addNeighbors(this.grid[i][j]);
                        }
                    }
    ...
    private void shuffleBomb(int bombCount) {
